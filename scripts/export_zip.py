...
EXPECTED_TOP_LEVEL = {
    ...
    # Report / dashboard summaries (generated by the reporting pipeline)
    "_meta.json",
    "articles_impact.json",
    "collab_summary.json",
    "egressos_summary.json",
    "facto_summary.json",
    "research_impact_summary.json",
    "researcher_impact.json",
}
...
def create_export_zip(
    ...
    if errors:
        ...
        if keep_on_error:
            print(
                "Keeping archive despite validation failures "
                "(--keep-on-error); complete validation happens at a later phase."
            )
        else:
            archive_path.unlink()
...
def main():
    ...
    parser.add_argument(
        "--keep-on-error",
        action="store_true",
        help="Keep the archive even when validation fails (e.g. intermediate snapshot)",
    )
    parser.add_argument(
        "--clean-loose",
        action="store_true",
